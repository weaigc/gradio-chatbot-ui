...
function IconButton(props: { icon: string; onClick?: () => void }) {
  return (
    ...
    >
      <Image alt="button" src={props.icon} className="w-6 h-6" />
    </div>
  )
}
...
function Sidebar() {
  ...
  return (
    ...
    >
      ...
      />
      {collapsed ? <Image alt="logo" src={minimalLogo} className="w-[30px]" /> : <Image alt="logo" src={logo} className="w-[79px]" />}
      <div className="flex flex-col gap-3 mt-2 overflow-y-auto scrollbar-none">
      ...
      <div className="mt-auto pt-2">
        ...
        {!collapsed && (
          <div className="my-5">
            <AddMore text={t('Add More')} />
          </div>
        )}
        <div className={cx('flex mt-5 gap-[10px] mb-4', collapsed ? 'flex-col' : 'flex-row ')}>
          {!collapsed && (
            <Tooltip content={t('GitHub')}>
              <a href="https://github.com/weaigc/gradio-chatbot?utm_source=webui" target="_blank" rel="noreferrer">
          ...
          {!collapsed && (
            <Tooltip content={t('Feedback')}>
              <a href="https://github.com/weaigc/gradio-chatbot/issues" target="_blank" rel="noreferrer">
